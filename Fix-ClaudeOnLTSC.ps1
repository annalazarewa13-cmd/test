<#
    ...
    ВНИМАНИЕ: у Claude Code CLI и десктопного приложения разные требования.
    CLI работает со сборки 17763 (LTSC 2019), приложению нужна 19041
    (Windows 10 2004). На LTSC 2019 приложение не установится никогда:
    номер сборки не меняется обновлениями, только переустановкой системы.
...
# --- Минимальные сборки. ВАЖНО: у CLI и десктопного приложения они РАЗНЫЕ ---
# CLI (claude в терминале): Windows 10 1809 / сборка 17763.
$MinBuild = 17763
# Десктопное приложение: Windows 10 2004 / сборка 19041. Установщик проверяет
# это жёстко и отказывается ставиться ниже. Обновлениями номер сборки не
# поднимается, поэтому на LTSC 2019 (17763) приложение недоступно в принципе.
$MinBuildDesktop = 19041
...
"@ -ForegroundColor Yellow
    ...
    Write-Ok "Сборка $build подходит для Claude Code CLI (минимум $MinBuild)."
}

if ($build -lt $MinBuildDesktop) {
    Write-Warn "Десктопное приложение Claude требует сборку $MinBuildDesktop (Windows 10 2004)."
    Write-Host '  На этой системе установщик откажет. Доступны CLI и claude.ai в браузере.' -ForegroundColor Yellow
    Write-Host '  Поднять номер сборки можно только in-place upgrade с ISO, не обновлениями.' -ForegroundColor Yellow
} else {
    Write-Ok "Сборка $build подходит и для десктопного приложения (минимум $MinBuildDesktop)."
}
